"""Проверка контаминации любой пары сплитов.
...
Четыре уровня, каждый ловит своё:
  ...
  3. группа  — вопросы одной статьи попали в разные сплиты;
  4. near-dup— близкие вопросы под разными id.
"""
...
from src.textnorm import normalize_group, question_text
...
def report(
    ...
    """Сводка пересечений двух сплитов. Ноль по всем ключам — сплит честный."""
    train_ids = {ex.id for ex in train}
    test_ids = {ex.id for ex in test}
    id_overlap = sorted(train_ids & test_ids)

    train_texts = [question_text(ex.user) for ex in train]
    test_texts = [question_text(ex.user) for ex in test]
